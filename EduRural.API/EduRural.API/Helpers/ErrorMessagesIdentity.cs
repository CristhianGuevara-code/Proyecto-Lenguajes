...
namespace EduRural.API.Helpers
{
    public class ErrorMessagesIdentity : IdentityErrorDescriber
    {
        ...
        public override IdentityError DuplicateEmail(string email) { return new IdentityError { Code = nameof(DuplicateEmail), Description = $"La dirección de email '{email}' ya se encuentra registrada. Puede recuperar su contraseña para ingresar nuevamente al sistema." }; }
        ...
        public override IdentityError PasswordRequiresNonAlphanumeric() { return new IdentityError { Code = nameof(PasswordRequiresNonAlphanumeric), Description = "La contraseña debe contener al menos un carácter alfanumérico." }; }
